...
describe('dashboard interactions', () => {
  it('exposes a complete accessible KPI label and emits selection', async () => {
    const wrapper = mount(KpiCard, {
      props: {
        kpi: { id: 'recall', label: '低活跃召回候选', displayValue: '504', trend: null, definition: '月度规则', tone: 'amber' },
      },
    })
    expect(wrapper.attributes('aria-label')).toContain('低活跃召回候选：504')
  ...
  it('closes the evidence drawer with Escape', async () => {
    const wrapper = mount(CdrDetailDrawer, {
      props: {
        record: {
          id: 'HV-ALL-001', direction: 'GD_TO_HK',
          focusType: '高用量服务保障', rule: 'Top 10%', phone: '131****6407', subscriberTier: 'Top 10% 高用量重点用户', status: '建议关怀',

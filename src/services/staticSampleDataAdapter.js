...
const PROFILE_SUMMARY = Object.freeze({
  highUsageUsers: 218,
  headTrafficShare: 0.9044392579485016,
  frequentTravelCandidates: 25,
  recallCandidates: 504,
  enterpriseCandidates: 26,
  highFrequencyLowVolumeUsers: 1,
  duplicateCandidateUsers: 358,
  growthCandidates: 253,
  companionGroupCandidates: 86,
})
...
function buildSummary(summary) {
  return {
    ...
    highUsageUsers: summary.top10Count,
    headTrafficShare: summary.top10TrafficShare,
  }
}
...
export class StaticSampleDataAdapter {
  async loadDashboardData(filters = { range: 'all', segment: SEGMENTS.ALL }) {
    ...
    const evidenceSummary = sourceSummary
    ...
    return {
      source: {
        kind: 'static-sample',
        label: '真实样本 · 静态脱敏聚合',
        message: '31,410 条记录 / 2,177 名用户',
        scope: '仅覆盖广东侧样本用户访问香港网络',
        disclaimer: '画像为行为规则推导，不代表身份、收入或满意度认定',
        version: 'v2026.08.02',
        snapshotAt: '2026-08-01 00:11:33',
      },
      filters: { ...filters, range },
      summary,
      kpis: createExperienceKpis(evidenceSummary, PROFILE_SUMMARY),
      trend: buildTrend(range, true),
      experienceSignals: buildExperienceSignals(),
      flows: [
        {
          direction: DIRECTIONS.GD_TO_HK,
          ...
          coverageNote: '当前样本仅覆盖单侧漫游行为，不用于双边结算稽核',
        },
      ],
      highValueRecords,
      recentFocusUsers: [...highValueRecords]
        .sort((left, right) => right.lastActiveDay.localeCompare(left.lastActiveDay))
        .slice(0, 7),
      pipeline: buildPipeline(sourceSummary),
      value: {
        highUsageUsers: PROFILE_SUMMARY.highUsageUsers,
        headTrafficShare: PROFILE_SUMMARY.headTrafficShare,
        recallCandidates: PROFILE_SUMMARY.recallCandidates,
        growthCandidates: PROFILE_SUMMARY.growthCandidates,
        enterpriseCandidates: PROFILE_SUMMARY.enterpriseCandidates,
        frequentTravelCandidates: PROFILE_SUMMARY.frequentTravelCandidates,
        companionGroupCandidates: PROFILE_SUMMARY.companionGroupCandidates,
        selectedSegment: filters.segment ?? SEGMENTS.ALL,
        note: '月度多标签行为规则结果，群体之间可重叠，不代表身份或收入认定',
      },

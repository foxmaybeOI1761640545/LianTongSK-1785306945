...
try {
  ...
  await page.getByRole('heading', { name: '粤港一卡双号用户体验与价值运营驾驶舱' }).waitFor()
  ...
  await page.getByTestId('sample-boundary-card').getByText('未提供对侧样本', { exact: true }).waitFor()
  await page.getByTestId('experience-path-panel').getByText('01 · 激活体验', { exact: true }).waitFor()
